...
export class UserInfoProfileComponent implements OnInit, AfterViewInit {
  @HostListener('window:scroll', ['$event']) onScrollEvent() {
    ...
    if (window.pageYOffset >= this.personalOffSet) {
      personal.classList.add('sticky-left');
      var parentwidth = document.getElementById("right-col").offsetTop;
      personal.style.top = parentwidth + 'px';
    } else {
      personal.classList.remove('sticky-left');
    }
  }
  ...
  adjustWidth() {
    const personal = document.getElementById('personal');
    var parentwidth = document.getElementById("right-col").clientWidth - 30;
    document.getElementById("right-container").style.width = parentwidth + 'px';
    personal.style.width = parentwidth + 'px';
  ...
  ngAfterViewInit() {
    this.userInfoService.getUserInfo(this.id).subscribe(userInfo => {
      ...
      this.nextGameList = userInfo.current_season.matches.slice(0, 5);
      console.log(this.nextGameList);
      this.session_user = this.authenticationService.getSessionUser();
      this.cdRef.detectChanges();
      this.loadChart();

      const personal = document.getElementById('personal');
      this.personalOffSet = personal.offsetTop + 20;
      this.adjustWidth();
    });
  }
...
//   var header = document.querySelector(".player-header-placeholder");
//   var navbar = document.getElementById('navbar');
//   var sticky = personal.offsetTop;
